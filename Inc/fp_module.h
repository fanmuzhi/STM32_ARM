...
typedef enum FlashReadType{ MISSION_FW, IOTA, FIB }flashRdType;
typedef enum FlashReadFlags{ BIGGEST, SHA256 }flashRdFlags;
//typedef enum FlashEraseType{ MISSION_FW, IOTA, MF_IOTA, FIB, SDB, ALL }flashEraseType;

typedef enum FrameTagType { FW_BL = 1, LNA_BL, DM, CDM, REG32BLK, REG16BLK, DIMS, ACQOPT, XSREG8BLK, STRIDEBLK, STATS, ACQCFG, ORIENTPITCH, IMGPROC_CROP, IMGPROC_DYNBPDET, IMGPROC_3X3SEPSQ, IMGPROC_DISABLE, EXTFPS_OTP_TEMPCORRECT, EXTFPS_OTP_DIEIMPRINT, EXTFPS_SIGCTRL, EXTFPS_SUNDRYREGS }frameTagType;

typedef enum WOFCalibrateType{ Baseline, Signal }wofCalibrateType;
